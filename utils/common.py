...
def cycle(dataloader, device, start_iteration: int = 0, type: str = "normal"):
    r"""
    ...
    while True:
        if isinstance(dataloader.sampler, torch.utils.data.DistributedSampler):
            ...
            logger.info(
                f"Beginning new epoch, setting shuffle seed {iteration}")
            dataloader.sampler.set_epoch(iteration)
            if type == "clusters":
                dataloader.sampler.dataset.update_iter(iteration)
...
def common_setup(_C: Config, _A: argparse.Namespace, job_type: str = "pretrain"):
    ...
    .. note::
        ...
        :mod:`utils.distributed` module ae used to get process info.
    Parameters
    ----------
    _C: config.Config
    ...
    os.makedirs(_A.checkpoints_dir + _C.RUN_ID, exist_ok=True)
    _C.dump(os.path.join(_A.checkpoints_dir +
            _C.RUN_ID, f"{job_type}_config.yaml"))
    ...
    if dist.get_world_size() > 1:
        logger.add(
            os.path.join(_A.checkpoints_dir + _C.RUN_ID,
                         f"log-rank{RANK}.txt"),
...
def common_parser(description: str = "") -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--checkpoints-dir", default="saves/checkpoints",
