...
class CheckpointManager(object):
    ...
    def climax_step(self, iteration: int):
        r"""
        Serialize checkpoint during last steps. Only saves the model params.
        ...
        checkpointable_state_dict: Dict[str,
                                        Any] = self._partial_state_dict("model")
    ...
    def _partial_state_dict(self, key: str):
        r"""Return a dict containing state dict of the key."""

        __partial_state_dict: Dict[str, Any] = {}

        if isinstance(
            self.checkpointables[key], nn.parallel.DistributedDataParallel
        ):
            __partial_state_dict[key] = self.checkpointables[key].module.state_dict(
            )
        else:
            __partial_state_dict[key] = self.checkpointables[key].state_dict()

        return __partial_state_dict
    ...
    def remove_earliest_checkpoint(self):
        ...
        (self.serialization_dir /
         f"checkpoint_{earliest_iteration}.pth").unlink()
