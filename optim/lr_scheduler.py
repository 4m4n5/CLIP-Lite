...
class LinearWarmupMultiStepLR(LambdaLR):
    ...
    def __init__(
        ...
        assert milestones == sorted(
            milestones), "milestones must be increasing"
    ...
    def _lr_multiplier(self, step: int) -> float:
        if step < self.wsteps:
            ...
            multiplier = self.gamma ** bisect.bisect_right(
                self.milestones, step)
...
class LinearWarmupCosineAnnealingLR(LambdaLR):
    ...
    def _lr_multiplier(self, step: int) -> float:
        ...
        # Avoid negative learning rate.
        return max(0, self.min_mult + multiplier)
